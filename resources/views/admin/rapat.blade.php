	...
	var urlView ="{{url('/admin/rapat-show-ruangan')}}";
	var urlSelect2 = "{{url(route('rapat.searchAjax'))}}";
    ...
    var listTable = $('.listTable').DataTable( {
        ...
        "ajax": {
         ...
         "columns": [
            ...
            { "render": function (data, type, row, meta) {
                	var app = [row.id_rapat];//approve
                	var pen = [row.id_rapat];//pending
                	var rej = [row.id_rapat];//reject
	                var approve = $('<a><button>')
	                            .attr('class', "btn btn-success  waves-effect edit-menu")
		                        .attr('onclick', "changeStatus('?id="+app+"&status="+1+"')")
	                            .text('Approve')
	                            .wrap('<div></div>')
	                            .parent()
	                            .html();
                    var pending = $('<button>')
		                        ...
		                        .attr('onclick', "changeStatus('?id="+pen+"&status="+2+"')")
		                        .text('Pending')
		                        .wrap('<div></div>')
		                        .parent()
		                        .html();
                    var reject = $('<button>')
		                        ...
		                        .attr('onclick', "changeStatus('?id="+rej+"&status="+3+"')")
		                        .text('Reject')
		                        .wrap('<div></div>')
		                        .parent()
		                        .html();
