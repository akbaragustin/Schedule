    ...
    public function save()
    {
        ...
        $messages=[
            'start_tgl_rapat.required'=>config('constants.ERROR_JML_WAJIB'),
             ...
             'pj_rapat.required'=>config('constants.ERROR_JML_WAJIB'),
             'status_ruangan_rapat.required'=>config('constants.ERROR_JML_WAJIB'),
          ];
           $validator=Validator::make(Input::all(), $rules, $messages);
           if ($validator->passes()) {
        $internal =new RP;
        $internal->agenda_rapat =Input::get('agenda_rapat');
        $internal->start_tgl_rapat =$start;
        $internal->end_tgl_rapat =$end;
        $internal->status_ruangan_rapat =Input::get('status_ruangan_rapat');
        if (!empty(Input::get('tempat_rapat'))) {
        $internal->tempat_rapat =Input::get('tempat_rapat');

        }
        $internal->pj_rapat =Input::get('pj_rapat');
        $internal->status_active_rapat =1;
        $internal->id_ruangan =Input::get('ruangan_rapat');
        $internal->save();

        $last_id = $internal->id_rapat;
...
public function updateStatus(){
        $id = Input::get('id');
    if (input::get('status') == 2 OR Input::get('status') == 3) {
        $check = RP::checkRapatStatus();
        if ($check[0]->status_active_rapat == 1) {
            $invite_jabatan =IJ::where('id_rapat',$id);
            $invite_jabatan->where('id_jabatan',15);
            $invite_jabatan->delete();
        }
    }

    $rapat = RP::find($id);
    $rapat->status_active_rapat =Input::get('status');
    $rapat->update();
    $last_id = $rapat->id_rapat;
    if (Input::get('status') == 1) {

        $queryEselon =SC::checkData($last_id);
             if (!empty($queryEselon)) {
                 $invite_jabatan_eselon = new IJ;
                 $invite_jabatan_eselon ->id_rapat =$last_id;
                 $invite_jabatan_eselon ->id_jabatan =15;
                 $invite_jabatan_eselon->save();
